...
function formatMessageDate(value) { if (!value) return ''; const date = new Date(value); if (Number.isNaN(date.getTime())) return ''; return date.toLocaleDateString('fr-FR', { day: '2-digit', month: '2-digit', year: 'numeric' }); }
...
function App() {
  ...
  const [guestbookMessages, setGuestbookMessages] = React.useState([]);
  ...
  async function loadData() {
    setLoading(true);
    const [{ data: betsData, error: betsError }, { data: resultData, error: resultError }, { data: guestbookData, error: guestbookError }] = await Promise.all([
      supabase.from('bets').select('*').order('created_at', { ascending: false }),
      supabase.from('game_result').select('*').eq('id', 1).single(),
      supabase.from('guestbook_messages').select('id, player, note, sort_order, created_at').order('sort_order', { ascending: true }),
    ]);
    ...
    if (guestbookError) { console.error('Erreur chargement des messages de la cagnotte', guestbookError); setGuestbookMessages([]); }
    else setGuestbookMessages(guestbookData || []);
    setLoading(false);
  }
  ...
  const messages = [
    ...bets.filter((item) => item.note).slice(0, 8).map((item) => ({ ...item, source: 'bet' })),
    ...guestbookMessages.map((item) => ({ ...item, source: 'guestbook' })),
  ];
  ...
  return <div className="app-shell">
    ...
    <main className="app-grid">
      ...
      <section className="message-wall-panel"><Title icon="💌" title="Mur des messages" sub="Les messages des paris et de la cagnotte sont réunis ici." /><div className="message-wall">{messages.length ? messages.map((item) => <article key={`${item.source}-${item.id}`} className="message-card"><div><b>{item.player}</b>{item.created_at && <small> · {formatMessageDate(item.created_at)}</small>}</div><p style={{ whiteSpace: 'pre-line' }}>“{item.note}”</p></article>) : <div className="empty-state">Aucun message pour l’instant.</div>}</div></section>
